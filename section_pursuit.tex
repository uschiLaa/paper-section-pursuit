...
University; $^{c}$Institute of Statistics, University of Natural
Resources and Life Sciences; $^{d}$Statistics Department, The Wharton
...
\subsubsection{\texorpdfstring{Generalised index
...
\hypertarget{the-index-in-practice}{%
\section{The index in practice}\label{the-index-in-practice}}
...
corners. This procedure is expected to be harmless because it is
interior structure that is of interest. In addition, this approach is
designed for reasonably small \(p\) so that the vast gap between spheres
and cubes in high dimensions is not a concern. For very high-dimensional
data, some dimension reduction is expected in pre-processing.
...
\hypertarget{dependence-on-q}{%
\subsubsection{\texorpdfstring{Dependence on
\(q\)}{Dependence on q}}\label{dependence-on-q}}
