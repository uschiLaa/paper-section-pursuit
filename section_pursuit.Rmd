...
\caption{Illustrations of slicing, through a 3D sphere (left), and demonstrating the calculation of the orthogonal distance (right). A slice can be done centred at the origin (green) or off-centre at $\mathbf{c}$ (blue). In more than three dimensions is best to use orthogonal distance between point and projection plane for computing the slice. Diagrams adapted from \cite{laa2019slice}.}
\label{fig:diagrams}
\end{figure*}
...
\textcolor{red}{Data points are thus classified in two ways: information about the projected position onto the viewing plane is captured by the binning, while information about the position in the orthogonal space is captured by slicing information. In practice the first step is projecting and slicing, followed by a separate binning of the projected points $Y$ for the subsets $S$ and $C$.}
...
```{r sketch, out.width="30%", fig.cap="Trying to sketch out binning, is this useful? What annotation should be added?", fig.align = "center"}
...
ggplot() +
  ...
  annotate("text", x = 0.3, y = 0.1, label=TeX("$r_i$"), size=15) +
