...
The definitions in Eq.(\ref{eq:index},\ref{eq:indexup}) can be generalized for example to emphasise selected bins or control the sensitivity.
\begin{equation}
...
This is because the marginal radial distribution of the projected data depends on the assumed reference distribution as well as the data dimenson $p$.
...
\label{eq:cdf}
\end{equation}
and depends on the hypersphere radius $R$ and on $p$. For illustration we show the CDF dependence on $r/R$ (where $0<r\leq R$) for different values of $p$ in Figure \ref{fig:cdf}. For large values of $p$ the majority of points is found within a small relative radius $r/R$, e.g. for $p=10$ we see that 75\% of points are within $r/R<1/2$.
...
- When binning we set $n_r=5$ and $n_{\theta}=8$ or $10$. \textcolor{red}{Here we have to consider the trade-off between resolution and required sample size. Note that the preferred relative resolution in radius and angle can be different depending on the input data and the type of structure. This typically requires testing different settings, similar to how we might look at histograms with different binnings to understand a distribution. Ideally we select the number of bins that allows to resolve the features in the data without introducing any artifacts.}
