...
This paper is organized as follows. The next section provides background on tours, slicing and projection pursuit. \textcolor{red}{Section \ref{sec:index} describes an index designed to detect concavities, and Section \ref{sec:practical} discusses the associated practical considerations and explores the behavior of the index on simulated examples.} The application  of section pursuit to exploring high-dimensional classification  models and constrained high-dimensional spaces is illustrateded in Section \ref{sec:applications}.  
...
library(tourr)
library(geozoo)
sphere3 <- geozoo::sphere.hollow(3, 5000)$points
colnames(sphere3) <- c("x1", "x2", "x3")
sphere3 <- sphere3 %>% as_tibble() %>%
  mutate(slice = ifelse(abs(x3) < 0.15, "inside", "outside"))
ggplot() +
